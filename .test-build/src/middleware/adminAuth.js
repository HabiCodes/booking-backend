...
async function adminAuthMiddleware(req, _res, next) {
    ...
    try {
        ...
        if (!decoded) {
            throw new errorHandler_1.AppError('Invalid admin token structure', 401);
        }
        // Verify admin account is still active
        const isActive = await verifyAdminIsActive(decoded.id);
        if (!isActive) {
            throw new errorHandler_1.AppError('Admin account has been deactivated', 401);
        }
        ...
        if (!permissionsFresh) {
            throw new errorHandler_1.AppError('Permissions have been updated — please re-authenticate', 401);
        }
        req.admin = decoded;
        next();
    }
