...
class PaymentService {
    ...
    async createOrder(input) {
        ...
        if (!organizationId)
            throw new errorHandler_1.AppError('Event has no organization', 400);
    ...
    async processRefund(input, actor) {
        return (0, pool_1.withTransaction)(async () => {
            const order = await paymentOrderRepository_1.paymentOrderRepository.findById(input.payment_order_id);
            if (!order)
                throw new errorHandler_1.AppError('Payment order not found', 404);
            ...
            if (Number(order.amount) < Number(input.amount)) {
                throw new errorHandler_1.AppError('Refund amount exceeds order amount', 400);
            }
            // Call gateway
            ...
            if (newAmount <= 0) {
                await paymentOrderRepository_1.paymentOrderRepository.updateStatus(order.id, 'REFUNDED');
            }
            else {
                await paymentOrderRepository_1.paymentOrderRepository.updateStatus(order.id, 'PARTIALLY_REFUNDED');
            }
    ...
    async _getTotalRefunded(paymentOrderId) {
        const refunds = await refundRepository_1.refundRepository.findByPaymentOrderId(paymentOrderId);
        return refunds
